      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div className="trending-grid">
          {trendingProducts.map((p) => {
            ...
            return (
              <div key={p.id} style={{ background: "#fff", border: "1px solid #eee", borderRadius: 10, padding: 16, display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: "#888" }}>{categoryLabel(p.category)}</span>
                  <button onClick={() => toggleWishlist(p.id, p.name)} style={{ background: "none", border: "1px solid #eee", borderRadius: "50%", width: 28, height: 28, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center", outline: "none" }} aria-label="Wishlist"><HeartIcon filled={isWishlisted(p.id)} /></button>
                </div>
